function  [Temp_Iter_Non, F,N, X] = PrePR_our( a, cin,cout,lambda, r, K )
% % %--------------------------------------------------------------------------------------------
%     PrePR_our: obtain the preprocessing time of PR_our algorithm 
%             a:  sparse matrix
...
% % %--------------------------------------------------------------------------------------------
    ...
    X = speye(r,r);                           % initialize the sparse identity matrix 
    idr = X; 
    DiagD1 = zeros(n,1);                      
    DiagD2 = DiagD1;                          % initialize the n-dimensional zeros vectors DiagD1 and DiagD2       
    for i = 1: K                              % # of subspace iterations 
        G1 = X * F';
        G2 = X * N';
        for j = 1:n
           DiagD1(j) = F(j,:) * G1(:,j);      
    ...
    clear H M  DiagD1  DiagD2 ;  
    
    Temp_Iter_Non = toc(time_Iter);        % Obtain the preprocessing time taken by the PR_our algorithm
